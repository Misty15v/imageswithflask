...
def upload_image():
    if 'image' not in request.files:
        return render_template('ImageML.html',prediction = 'No posted image.Should be attribute name image')
    if file.filename == '':
        return render_template('ImageML.html',Iprediction = 'You did not select an image')
    if file and allowed_file(file.filename):
        ...
        items = []
        for item in lst[0]:
            item.append({'name':item[1],'prob':float(item[2])})
        response = {'pred':items}
